...
async function handleSequence(value: any, rules: Array<Rule>, _value: any, keepInitialValue?: boolean) {
    let res: any = value;
...
async function handleRecord(value: any, rules: Record<string, Rule>, _value: any, discardPreviousSteps?: boolean) {
    const res = {
        ...(discardPreviousSteps || ensureIsPrimitive(value) ? {} : value),
        ...Object.fromEntries(
            await Promise.all(Object.entries(rules).map(async ([key, rule]) => ([key, await _parse(rule, value, _value)])))
        )
    }
    log("record", res)
    return res
...
async function _parse(rule: Rule, value: any, _value: any) {
    ...
    switch (rule.method) {
        ...
        case "html-query-selector":
            res = (value as HTMLElement)?.querySelector(normalizeMaybeMethod(rule.selector, value, _value))
            break
        case "html-query-selector-all":
            res = (value as HTMLElement)?.querySelectorAll(normalizeMaybeMethod(rule.selector, value, _value))
            break
        case "html-text":
            res = (value as HTMLElement)?.text?.trim()
            break
        case "html-attribute":
            res = (value as HTMLElement)?.getAttribute(normalizeMaybeMethod(rule.attribute, value, _value))
            break
        ...
        // meta rules
        case "iterate":
            res = await Promise.all(value.map((v: any) => _parse(
                rule.iterate,
                v,
                rule.setCurrentItemAsInitialValue ? v : _value
            )))
            break;
        case "sequence":
...
function ensureIsPrimitive(value: any): value is Primitive {
    return value === null || ["undefined", "boolean", "number", "bigint", "string", "symbol"].includes(typeof value)
}
